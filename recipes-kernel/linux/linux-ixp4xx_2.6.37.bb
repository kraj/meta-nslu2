...
do_postpatch() {
	...
	[ -e ${S}/patches ] && mv ${S}/patches ${S}/patches.oe
	[ -e ${S}/.pc ]     && mv ${S}/.pc ${S}/.pc.oe

	# Move the NSLU2 patches in place for quilt and apply them.
        mv ${WORKDIR}/${VANILLA_VERSION} ${S}/patches && cd ${S} && quilt push -av && cd -
	...
	[ -e ${S}/patches.oe ] && mv ${S}/patches.oe ${S}/patches
	[ -e ${S}/.pc.oe ]     && mv ${S}/.pc.oe ${S}/.pc
