...
SRCREV = "e2fefd5b86aa468db304f14ef7fb317f9d12ae83"

PV = "1.6.9+git${SRCPV}"

SRC_URI = "git://github.com/beewoolie/apex.git;protocol=git;branch=master \
           file://0001-some-more-dangling-references-to-aliases-replaced-wi.patch \
	   file://defconfig"
S = "${WORKDIR}/git"
...
do_install() {
	install -d ${D}/loader
	if [ "x${SITEINFO_ENDIANNESS}" = "xbe" ]; then
		install -m 0644 src/arch-arm/rom/apex.bin ${D}/loader/apex-nslu2.bin
	elif [ "x${SITEINFO_ENDIANNESS}" = "xle" ]; then
		devio '<<'src/arch-arm/rom/apex.bin >${D}/loader/apex-nslu2.bin 'xp $,4'
	else
